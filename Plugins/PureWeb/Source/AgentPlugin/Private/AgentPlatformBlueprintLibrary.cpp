...
#include "..\Public\AgentPlatformBlueprintLibrary.h"
const FString ENV_STREAM_RES_X = "PixelStreamingResX=";
const FString ENV_STREAM_RES_Y = "PixelStreamingResY=";
...
void UAgentPlatformBlueprintLibrary::InitializeResolution()
{
	FString resX;
	FString resY;
	
	FParse::Value(FCommandLine::Get(), *ENV_STREAM_RES_X, resX, false);
	FParse::Value(FCommandLine::Get(), *ENV_STREAM_RES_Y, resY, false);
	if (resX.IsEmpty()) {
		resX = "1920";
	}
	if (resY.IsEmpty()) {
	...
	FCString::Atoi(*resX);
	GEngine->GameViewport->ViewportFrame->ResizeFrame(FCString::Atoi(*resX), FCString::Atoi(*resY), EWindowMode::Fullscreen);
